...
from cheese import Move, Square, Type
from cheese.board import Board
from cheese.movegen import Movegen


def assert_eq_after_move(initial_fen: str, move: Move, expected_fen: str) -> None:
    initial = Board.from_fen(initial_fen)
    initial.make_move(move)
    final = Board.from_fen(expected_fen)
    assert initial == final


# TODO: Remove this monkeypatch once we have a working movegenerator.
# Then add some tests to ensure illegal moves are rejected
# (illegal target move / trying to move after game already ended).
@pytest.fixture(autouse=True)
def turn_all_moves_legal(monkeypatch: MonkeyPatch) -> None:
    def generate_legal_moves(board: Board, square: Square) -> list[Move]:
        source_piece = board.get_piece(square)
        ...
        for i in range(64):
            ...
            for j in range(64):
                if i == j:
                    continue
                target = Square(j)
                ret.append(Move(source, target))
                if source_piece.ptype == Type.PAWN and j // 8 in {7, 0}:
                    ret.append(Move(source, target, promotion=Type.QUEEN))
                    ret.append(Move(source, target, promotion=Type.ROOK))
                    ret.append(Move(source, target, promotion=Type.KNIGHT))
                    ret.append(Move(source, target, promotion=Type.BISHOP))
        return ret

    monkeypatch.setattr(
        Movegen, Movegen.generate_legal_moves.__name__, generate_legal_moves
    )
...
def test_regular_moves(initial_fen: str, move: Move, expected_fen: str) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "rnbqkbnr/ppp2ppp/7B/3pp3/3PP3/8/PPP2PPP/RN1QKBNR b KQkq - 1 3",
            Move(Square.g7, Square.h6),
            "rnbqkbnr/ppp2p1p/7p/3pp3/3PP3/8/PPP2PPP/RN1QKBNR w KQkq - 0 4",
        ),
    ],
)
def test_captures(initial_fen: str, move: Move, expected_fen: str) -> None:
    assert_eq_after_move(initial_fen, move, expected_fen)
...
def test_castling(initial_fen: str, move: Move, expected_fen: str) -> None:
    assert_eq_after_move(initial_fen, move, expected_fen)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            Move(Square.g7, Square.f8, promotion=Type.QUEEN),
        ...
        (
            ...
            Move(Square.g2, Square.f1, promotion=Type.KNIGHT),
            "rnbqkbnB/pppppp1p/8/8/8/8/PPPPPP1P/RNBQKnNR w KQq - 0 2",
        ),
    ],
)
def test_promotions(initial_fen: str, move: Move, expected_fen: str) -> None:
    assert_eq_after_move(initial_fen, move, expected_fen)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "rnbqkbnr/pppppp1p/8/8/6pP/8/PPPPPPP1/RNBQKBNR b KQkq h3 0 1",
            Move(Square.g4, Square.h3),
            "rnbqkbnr/pppppp1p/8/8/8/7p/PPPPPPP1/RNBQKBNR w KQkq - 0 2",
        ),
    ],
)
def test_en_passant(initial_fen: str, move: Move, expected_fen: str) -> None:
    assert_eq_after_move(initial_fen, move, expected_fen)
